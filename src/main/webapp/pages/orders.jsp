...
<head>
	...
	<script type="text/javascript">
		function deleteById(id,username) {
			alert("id"+id);
			window.location.href="${pageContext.request.contextPath }/order/toDeleteOrders.action?id="+id+"&username="+username;
		}
		function editOrder(id,username) {
			alert("id"+id);
			window.location.href="${pageContext.request.contextPath }/order/toUpdateOrders.action?id="+id+"&username="+username;
		}
		function addOrders(username) {
			window.location.href="${pageContext.request.contextPath }/order/toAddOrder.action?username="+username;
			...
			<div class="page-header">
				<div class="row align-items-center">
					<div class="col">
						...
						<th><button class="btn btn-sm bg-success-light mr-2" id="edit_Account_btn1" onclick = "addOrders('${manager.username}')"><i class="fe fe-pencil"></i>添加订单</button></th>
						<ul class="breadcrumb">
						...
						<div class="card-body">
							<div class="table-responsive">
								<table class="table table-hover table-center mb-0">
									...
									<c:forEach items="${orders}" var="order" varStatus="status">
										...
										<ul>
											...
											<tr>
												...
												<th>${order.userId }</th>
												<th>${order.cwId }</th>
												<th>${order.ordersMoney }</th>
												<th>${order.ordersTime }</th>
												<th>${order.ordersEval }</th>
												<th>${order.ordersStatus}</th>
												<th><button class="btn btn-sm bg-danger-light" onclick = "deleteById('${order.ordersId}','${manager.username}')"><i class="fe fe-trash"></i>删除</button></th>
												<th><button class="btn btn-sm bg-success-light mr-2" id="edit_Account_btn" onclick = "editOrder('${order.ordersId}','${manager.username}')"><i class="fe fe-pencil"></i>编辑</button></th>
											</tr>
									...
									<ul>
										<th>
											<a href="${pageContext.request.contextPath }/order/toListOrders?pn=1&username=${manager.username}">1</a>
										</th>
										<th>
											<a href="${pageContext.request.contextPath }/order/toListOrders?pn=2&username=${manager.username}">2</a>
										</th>
										...
										<th>
											<a href="${pageContext.request.contextPath }/order/toListOrders?pn=${orders1.pageNum-1}&username=${manager.username}">上一页</a>
										</th>
										<th>
											<a href="${pageContext.request.contextPath }/order/toListOrders?pn=${orders1.pages}&username=${manager.username}">尾页</a>
										</th>
										<th>
											<a href="${pageContext.request.contextPath }/order/toListOrders?pn=${orders1.pageNum+1}&username=${manager.username}">下一页</a>
										</th>
